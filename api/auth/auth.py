...
def login(req: LoginRequest, db: Session = Depends(get_db)):
    try:
        ...
        if user is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Username does not exists.")
        
        if not verify_password(req.password, user.hashed_password):
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Username or password is incorrect.")

        token = create_access_token({"sub": req.username})
        return {"access_toekn": token, "type": "bearer"}
    except:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, details="fail to login.")
    
oauth_scheme = OAuth2PasswordBearer(tokenUrl="/login")

def get_current_user(token: Users = Depends(oauth_scheme), db: Session = Depends(get_db)):
